<template>
    <el-result v-if="authRequired" icon="warning" title="Внимание"
             sub-title="Для просмотра черного списка необходимо авторизоваться."/>
    <div v-else>
        <el-text :size="'large'" class="title">Ваш черный список</el-text>
        <el-text type="info" class="subtitle">
            Это пользователи, которые не будут иметь доступа к вашим постам.
        </el-text>
...
async function removeUser(id) {
    try {
        const res = await fetch(`/api/accounts/lists/blacklist/${id}/`, {
            method: 'DELETE',
            credentials: 'include'
        })
        const data = await res.json()
        ...
        ElMessage({
            message: data.message || 'Пользователь удален из черного списка',
            type: 'success',
        })
    } catch (e) {
        ElMessage({
